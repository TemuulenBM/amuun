...
export function DestinationsHero({ eyebrow, heading, intro }: DestinationsHeroProps) {
  ...
  useEffect(() => {
    ...
    if (!section || !headline || !eyebrowEl || !introEl) return;
  ...
  // Split heading into italic first clause + bold remainder (tour-hero brand pattern)
  const splitIndex = heading.includes(',') ? heading.indexOf(',') + 1 : heading.indexOf(' ');
  const firstPart = splitIndex > 0 ? heading.slice(0, splitIndex) : heading;
  const secondPart = splitIndex > 0 ? heading.slice(splitIndex) : '';
